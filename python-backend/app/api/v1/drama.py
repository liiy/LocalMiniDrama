...
def create_drama(payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    body = payload or {}
...
def save_outline(drama_id: str, payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    ok = svc.save_outline(db, drama_id, payload or {})
    if not ok:
...
def save_characters(drama_id: str, payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    body = payload or {}
...
def save_episodes(drama_id: str, payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    body = payload or {}
...
def save_progress(drama_id: str, payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    body = payload or {}
...
def save_canvas_layout(drama_id: str, payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    try:
        updated = svc.save_canvas_layout(db, drama_id, payload or {})
...
def update_drama(drama_id: str, payload: dict = Body(default={}), db: Session = Depends(get_db)) -> dict:
    drama = svc.update_drama(db, drama_id, payload or {})
    if not drama:
        raise not_found("剧本不存在")
    return success(drama)
