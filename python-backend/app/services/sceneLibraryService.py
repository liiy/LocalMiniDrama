"""场景素材库 CRUD（等价 Node services/sceneLibraryService.js 的纯 CRUD 部分）。

P2 阶段只翻译库表自身的增删改查；从场景同步入库等流程留到 P4。
"""
...
from app.services import sceneEntityService as scene_svc
...
log = get_logger("lmd.scene_library")

TABLE = "scene_libraries"
KEYWORD_COLS = ["location", "description", "prompt"]
...
def scene_library_fields(scene: dict, drama_id, image_url: str | None, now: str) -> dict:
    """等价 Node sceneLibraryFields：组装写入 scene_libraries 的字段。"""
    return {
        "drama_id": drama_id,
        "location": scene.get("location") or "",
        "time": scene.get("time") or None,
        "prompt": scene.get("prompt") or None,
        "description": scene.get("prompt") or None,
        "image_url": image_url,
        "local_path": scene.get("local_path") or None,
        "source_type": "scene",
        "source_id": normalize_source_id(scene.get("id")),
        "updated_at": now,
    }


def add_scene_to_library(db: Session, log, scene_id) -> dict:
    """等价 Node addSceneToLibrary：把场景同步进本剧素材库（纯 DB 关联，无 AI）。"""
    scene = scene_svc.get_scene_by_id(db, to_int_id(scene_id))
    if not scene:
        return {"ok": False, "error": "scene not found"}
    drama = db.execute(
        text("SELECT id FROM dramas WHERE id = :id AND deleted_at IS NULL"), {"id": scene.get("drama_id")}
    ).mappings().first()
    if not drama:
        return {"ok": False, "error": "unauthorized"}
    if not scene.get("image_url") and not scene.get("local_path"):
        return {"ok": False, "error": "场景还没有形象图片"}
    now = timestamp()
    image_url = resolve_image_url(scene.get("image_url"), scene.get("local_path"))
    fields = scene_library_fields(scene, scene.get("drama_id"), image_url, now)
    existing = find_existing_library_item(
        db, TABLE, drama_id=scene.get("drama_id"), source_type="scene", source_id=scene.get("id"),
        image_url=image_url, local_path=scene.get("local_path"),
    )
    if existing:
        update_existing_library_item(db, TABLE, existing["id"], fields)
        log.info("Scene library item reused", extra={"scene_id": scene_id, "library_item_id": existing["id"]})
    ...
    log.info("Scene added to drama library", extra={"scene_id": scene_id, "library_item_id": info.lastrowid})
    return {"ok": True, "item": get_library_item(db, str(info.lastrowid)), "duplicated": False}


def add_scene_to_material_library(db: Session, log, scene_id) -> dict:
    """等价 Node addSceneToMaterialLibrary：把场景同步进全局素材库（drama_id=NULL）。"""
    scene = scene_svc.get_scene_by_id(db, to_int_id(scene_id))
    if not scene:
        return {"ok": False, "error": "scene not found"}
    if not scene.get("image_url") and not scene.get("local_path"):
    ...
    now = timestamp()
    image_url = resolve_image_url(scene.get("image_url"), scene.get("local_path"))
    fields = scene_library_fields(scene, None, image_url, now)
    existing = find_existing_library_item(
        db, TABLE, drama_id=None, source_type="scene", source_id=scene.get("id"),
        image_url=image_url, local_path=scene.get("local_path"),
    )
    if existing:
        update_existing_library_item(db, TABLE, existing["id"], fields)
        log.info("Scene material library item reused", extra={"scene_id": scene_id, "library_item_id": existing["id"]})
        return {"ok": True, "item": get_library_item(db, str(existing["id"])), "duplicated": True}
    info = insert_library_item(db, TABLE, {**fields, "created_at": now})
    log.info("Scene added to material library (global)", extra={"scene_id": scene_id, "library_item_id": info.lastrowid})
    return {"ok": True, "item": get_library_item(db, str(info.lastrowid)), "duplicated": False}
...
def create_library_item(db: Session, req: dict) -> dict:
    ...
    log.info("Scene library item created", extra={"item_id": info.lastrowid})
...
def update_library_item(db: Session, item_id, req: dict) -> dict | None:
    ...
    log.info("Scene library item updated", extra={"item_id": item_id})
    return get_library_item(db, item_id)
...
def delete_library_item(db: Session, item_id) -> bool:
    ...
    if result.rowcount == 0:
        return False
    log.info("Scene library item deleted", extra={"item_id": item_id})
    return True
