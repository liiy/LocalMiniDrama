...
def _is_english(cfg: dict | None) -> bool:
    if not cfg:
        cfg = load_config()
    return str((cfg or {}).get("app", {}).get("language") or "zh").lower() == "en"


def get_story_expansion_system_prompt(cfg: dict | None, episode_count: int | str | None) -> str:
    n = int(episode_count) if str(episode_count).strip() not in ("", "0") else 1
    n = max(1, n)
    json_note = (
        f"\n\n**输出格式（必须严格遵守）**：\n"
        f"返回一个 JSON 数组，包含 {n} 个对象，每个对象格式如下：\n"
    ...
    if _is_english(cfg):
        en_note = (
            ...
            f"Return a JSON array with {n} object(s), each in this format:\n"
        ...
        return (
            f"You are a professional screenwriter. Your task is to expand the user's story premise into {n} episode(s) of a short-film script.\n\n"
            "Requirements:\n"
            ...
            "4. Each episode should have a clear beginning, development, and a hook or turning point at the end."
            f"{en_note}"
        )
    base = _override_cache.get("story_expansion_system") or DEFAULT_BODIES.get("story_expansion_system", "")
    return base + json_note
...
def build_story_expansion_user_prompt(cfg: dict | None, premise: str, style: str | None, type_: str | None, episode_count: int | str | None) -> str:
    lang = "en" if _is_english(cfg) else "zh"
    n = int(episode_count) if str(episode_count).strip() not in ("", "0") else 1
    n = max(1, n)
    style_labels = _story_style_labels[lang]
    type_labels = _story_type_labels[lang]
    if lang == "en":
        prompt = f"Please create {n} episode(s) of a short-film script based on the following story premise:\n\n{premise}"
        if style and style_labels.get(style):
            prompt += f"\n\nStyle: {style_labels[style]}"
        if type_ and type_labels.get(type_):
            prompt += f"\n\nGenre: {type_labels[type_]}"
        if n > 1:
            prompt += f"\nEpisodes: {n}"
        return prompt
    prompt = f"请根据以下故事梗概，创作 {n} 集短片剧本：\n\n{premise}"
    if style and style_labels.get(style):
        prompt += f"\n\n故事风格：{style_labels[style]}"
    if type_ and type_labels.get(type_):
        prompt += f"\n\n剧本类型：{type_labels[type_]}"
    if n > 1:
        prompt += f"\n生成集数：{n} 集"
    return prompt


def get_regenerate_layout_description_prompt(cfg: dict | None) -> str:
    if _is_english(cfg):
        return (
            "You are a professional film continuity supervisor and storyboard spatial designer.\n\n"
...
def get_language(cfg: dict | None) -> str:
    if not cfg:
        return "zh"
    return str((cfg.get("app") or {}).get("language") or "zh").lower()


def is_english(cfg: dict | None) -> bool:
    return get_language(cfg) == "en"


def style_text_for_cfg_lang(cfg: dict | None) -> str:
    if not cfg:
        return ""
    style = cfg.get("style") or {}
    z = (style.get("default_style_zh") or "").strip()
    e = (style.get("default_style_en") or "").strip()
    d = (style.get("default_style") or "").strip()
    if is_english(cfg):
        return e or d
    return z or d


def style_text_zh_for_polish(cfg: dict | None) -> str:
    if not cfg:
        return ""
    style = cfg.get("style") or {}
    return (style.get("default_style_zh") or style.get("default_style") or "").strip()


def style_text_en_for_image(cfg: dict | None) -> str:
    if not cfg:
        return ""
    style = cfg.get("style") or {}
    return (style.get("default_style_en") or style.get("default_style") or "").strip()


def get_identity_anchors_prompt() -> str:
...
def get_character_extraction_prompt(cfg: dict | None) -> str:
    style = style_text_for_cfg_lang(cfg)
    image_ratio = ((cfg.get("style") or {}).get("default_image_ratio") or "16:9") if cfg else "16:9"
    if is_english(cfg):
        return f"""You are a professional character analyst, skilled at extracting and analyzing character information from scripts.
...
Each element is a character object containing the above fields."""
    _char_override = _override_cache.get("character_extraction")
    if _char_override:
        return f"{_char_override}\n- **风格要求**：{style}\n- **图片比例**：{image_ratio}\n输出格式：\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块、说明文字或其他内容。直接以 [ 开头，以 ] 结尾。**\n每个元素是一个角色对象，包含上述字段。"
    return f"""你是一个专业的角色分析师，擅长从剧本中提取和分析角色信息。
...
def get_scene_extraction_prompt(cfg: dict | None, style: str | None = None) -> str:
    style_text = (style or "").strip()
    s = style_text or style_text_for_cfg_lang(cfg)
    image_ratio = ((cfg.get("style") or {}).get("default_image_ratio") or "16:9") if cfg else "16:9"
    if is_english(cfg):
...
Each element: location, time, prompt (English image generation prompt for pure background)."""
    _scene_locked = f"\n5. **风格要求**：{s}\n   - **图片比例**：{image_ratio}\n\n【输出格式】\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块。直接以 [ 开头，以 ] 结尾。**\n每个元素包含：location（地点）, time（时间）, prompt（完整的中文图片生成提示词，纯背景，明确说明无人物）。"
    _scene_override = _override_cache.get("scene_extraction")
    if _scene_override:
        return _scene_override + _scene_locked
...
def get_role_polish_prompt(cfg: dict | None) -> str:
    style = style_text_zh_for_polish(cfg)
    style_rule = f"\n- **画风风格（须贯穿各栏描述，与下长生图侧画风块一致）**：{style}" if style else ""
    return f"""# 角色多视图设定图提示词生成器
...
- **时代匹配**：服装与发型必须符合作品类型所属时代背景{style_rule}
...
def get_role_generate_image_prompt() -> str:
    return ""


def get_scene_polish_prompt_single(cfg: dict | None) -> str:
    style = style_text_zh_for_polish(cfg)
    style_rule = f"\n- **画风风格**：{style}" if style else ""
    return f"""# 场景单图参考图生成器
...
- **真实可信**：建筑风格、材质、植被必须符合场景所属时代和地域{style_rule}
...
def get_scene_polish_prompt(cfg: dict | None) -> str:
    style = style_text_zh_for_polish(cfg)
    style_rule = f"\n- **画风风格**：{style}" if style else ""
    return f"""# 场景四视图参考图生成器
...
def get_scene_generate_image_prompt() -> str:
...
def get_scene_generate_single_image_prompt() -> str:
...
def get_prop_extraction_prompt(cfg: dict | None) -> str:
    base = style_text_for_cfg_lang(cfg)
    prop_extra = ((cfg.get("style") or {}).get("default_prop_style") or "").strip() if cfg else ""
    style = ", ".join(filter(None, [base, prop_extra]))
    image_ratio = ((cfg.get("style") or {}).get("default_prop_ratio") or (cfg.get("style") or {}).get("default_image_ratio") or "16:9") if cfg else "16:9"

    if is_english(cfg):
        return f"""You are a professional script prop analyst, skilled at extracting key props with visual characteristics from scripts.
...
- image_prompt: English hero product shot prompt (single prop, solid seamless backdrop, no clutter, no environment, soft studio light, tight wording, no names/places from script, ultra-detailed only where visually grounded)"""
    ...
    prop_override = _override_cache.get("prop_extraction")
    ...
    if prop_override:
        return prop_override + prop_locked

    return f"""你是一位专业的剧本道具分析师，擅长从剧本中提取具有视觉特征的关键道具。
...
def get_prop_polish_prompt(cfg: dict | None) -> str:
    style_zh = style_text_zh_for_polish(cfg)
    style_en = style_text_en_for_image(cfg)
    if is_english(cfg):
        style_zh_rule = f"\n- **画风风格**（仅作用于渲染质感，不改变「单道具 + 纯色底」版式）：{style_zh}" if style_zh else ""
        end_style = f"{style_en} render style" if style_en else "photorealistic product hero shot"
        return f"""# 道具图片提示词生成器
...
### 硬性排除
- 禁止：人物、手、身体任何部分、文字水印、商标（除非剧情指定且为道具本体一部分）、叙事性场景词、**任何专有名词式剧本标签**。{style_zh_rule}

### 输出格式
直接输出**一段**英文 prompt（约 **45–90 词**，能更短则更短），不要解释、标题、列表或引号。
**必须**在同一段内显式包含短语或等价表达：**single prop only**, **seamless solid-color studio backdrop**, **no extra objects**, **no people**, **no hands**, **no environment**；末尾再接画风：{end_style}"""

    style_zh_rule = f"\n- **画风风格**（仅作用于渲染质感，不改变「单道具 + 纯色底」版式）：{style_zh}" if style_zh else ""
    end_style = f"{style_zh} 渲染质感" if style_zh else "写实产品主图质感"
    return f"""# 道具图片提示词生成器（中文版）
...
**必须**在同一段内自然包含以下关键约束的中文表述（或等价流畅说法）：单一主体、纯色无缝棚拍背景、无多余物体、无人物、无手、无环境；并融入真实尺度与次要元素要求；末尾再接画风：{end_style}"""


def get_storyboard_system_prompt(cfg: dict | None) -> str:
    if is_english(cfg):
...
- segment_index must be sequential integers starting from 0; all shots in the same segment share the same index and title"""
    ...
    override = _override_cache.get("storyboard_system")
    if override:
        return (
            override
            + "\n\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块、说明文字或其他内容。直接以 [ 开头，以 ] 结尾。**\n\n【重要提示】\n- 镜头数量必须与剧本中的独立动作数量匹配（不允许合并或减少）\n- 每个镜头必须有明确的动作和结果\n- 景别选择必须符合叙事节奏（不要连续使用同一景别）\n- 情绪强度必须准确反映剧本氛围变化"
...
def get_universal_omni_multi_beat_format_spec(cfg: dict | None) -> str:
    from app.services.universalOmniMultiBeatFormat import DEFAULT_LINE3
    if is_english(cfg):
        return f"""
...
def get_universal_omni_segment_prompt(cfg: dict | None = None) -> str:
    spec_zh = get_universal_omni_multi_beat_format_spec({"general": {"language": "zh"}})
    return f"""You write the main prompt for multi-reference video (e.g. Kling Omni-Video, Volcengine Seedance omnivideo) "片段描述" in Chinese.
...
def get_universal_omni_polish_prompt(cfg: dict | None = None) -> str:
    base = get_universal_omni_segment_prompt(cfg)
    return f"""{base}
...
def get_storyboard_universal_omni_mode_suffix(cfg: dict | None) -> str:
    spec = get_universal_omni_multi_beat_format_spec(cfg)
    if is_english(cfg):
        return f"""
...
def get_storyboard_narration_extra_instructions(cfg: dict | None) -> str:
    if is_english(cfg):
        return """
...
def format_user_prompt(cfg: dict | None, key: str, *args: Any) -> str:
    style = style_text_for_cfg_lang(cfg)
    image_ratio = (cfg.get("style") or {}).get("default_image_ratio") if cfg else "16:9"
    ...
    lang = "en" if is_english(cfg) else "zh"
    t = templates.get(lang, {}).get(key) or templates["zh"].get(key)
    if not t:
    ...
    def repl(m: re.Match) -> str:
        ...
        if idx < len(args) and args[idx] is not None:
            ...
            return res
        return ""
    return re.sub(r"%[sd]", repl, t)


def get_storyboard_user_prompt_suffix(cfg: dict | None, shot_duration: float | int | None = None) -> str:
    lang = "en" if is_english(cfg) else "zh"
    duration_hint = None
    try:
        if shot_duration is not None and float(shot_duration) > 0:
            duration_hint = float(shot_duration)
    except Exception:
        pass

    if lang == "en":
        duration_instruction = (
            f"approximately {duration_hint}s per shot (project setting), adjust ±1s based on dialogue length and action complexity"
            if duration_hint
            else "estimate per shot from dialogue length, action complexity, and emotion"
        )
        return f"""
...
**duration (seconds)**: {duration_instruction}.
...
**Output**: JSON with "storyboards" array. Each item: shot_number, segment_index, segment_title, title, shot_type, angle, time, location, scene_id, movement, action, dialogue, result, atmosphere, emotion, duration, bgm_prompt, sound_effect, characters (array of IDs), props (array of prop IDs), is_primary. Return ONLY valid JSON, no markdown."""
    ...
    sb_user_override = _override_cache.get("storyboard_user_suffix")
    if sb_user_override:
        return "\n\n" + sb_user_override + sb_user_locked

    duration_instruction = (
        f"每镜头约{duration_hint}秒（项目配置），综合对话、动作、情绪可适当调整±1秒"
        if duration_hint
        else "综合对话、动作、情绪估算每镜时长（秒）"
    )
    return f"""
...
**dialogue字段说明**：角色名："台词内容"。无对话时填空字符串""。
**scene_id**：从上方场景列表中选择最匹配的背景ID，如无合适背景则填null。
**duration时长**：{duration_instruction}。
...
def get_realistic_physical_scale_contract(is_en: bool) -> str:
    if is_en:
...
def get_first_frame_prompt(cfg: dict | None) -> str:
    style = style_text_en_for_image(cfg) if is_english(cfg) else style_text_zh_for_polish(cfg)
    image_ratio = (cfg.get("style") or {}).get("default_image_ratio") if cfg else "16:9"
    image_ratio = image_ratio or "16:9"
    if is_english(cfg):
        return f"""You are a professional cinematic storyboard image prompt expert. Generate AI image generation prompts based on the shot information provided.
...
- description: Simplified Chinese description (for reference)"""
    ...
    ff_locked = f"\n- **风格要求**：{style}\n- **图片比例**：{image_ratio}\n输出格式：\n返回一个JSON对象，包含：\n- prompt：完整的中文图片生成提示词（详细的电影语言描述）\n- description：简化的中文描述（供参考）"
    ff_override = _override_cache.get("first_frame_prompt")
    if ff_override:
        return ff_override + ff_locked

    ff_scale_contract = get_realistic_physical_scale_contract(False)
    return f"""你是一个专业的电影分镜图像生成提示词专家。请根据提供的镜头信息，生成适合AI图像生成的提示词。
...
第5层-视觉风格：{f'{style}，' if style else ''}电影分镜质感，{image_ratio} 画幅，高清细节，所有物体严格真实尺度
...
def get_key_frame_prompt(cfg: dict | None) -> str:
    style = style_text_en_for_image(cfg) if is_english(cfg) else style_text_zh_for_polish(cfg)
    image_ratio = (cfg.get("style") or {}).get("default_image_ratio") if cfg else "16:9"
    image_ratio = image_ratio or "16:9"
    if is_english(cfg):
...
- description: Simplified Chinese description (for reference)"""
    ...
    kf_locked = f"\n- **风格要求**：{style}\n- **图片比例**：{image_ratio}\n输出格式：\n返回一个JSON对象，包含：\n- prompt：完整的中文图片生成提示词（详细的电影语言描述）\n- description：简化的中文描述（供参考）"
    kf_override = _override_cache.get("key_frame_prompt")
    if kf_override:
        return kf_override + kf_locked
...
第5层-视觉风格：{f'{style}，' if style else ''}电影分镜质感，{image_ratio} 画幅，动态张力，所有物体严格真实尺度
...
def get_last_frame_prompt(cfg: dict | None) -> str:
    style = style_text_en_for_image(cfg) if is_english(cfg) else style_text_zh_for_polish(cfg)
    image_ratio = (cfg.get("style") or {}).get("default_image_ratio") if cfg else "16:9"
    image_ratio = image_ratio or "16:9"
    if is_english(cfg):
...
- description: Simplified Chinese description (for reference)"""
    ...
    lf_locked = f"\n- **风格要求**：{style}\n- **图片比例**：{image_ratio}\n输出格式：\n返回一个JSON对象，包含：\n- prompt：完整的中文图片生成提示词（详细的电影语言描述）\n- description：简化的中文描述（供参考）"
    lf_override = _override_cache.get("last_frame_prompt")
    if lf_override:
        return lf_override + lf_locked
...
第4层-氛围：情绪基调 + 色彩倾向
第5层-视觉风格：{f'{style}，' if style else ''}电影分镜质感，{image_ratio} 画幅，所有物体严格真实尺度，运镜自然演化
...
styleTextForCfgLang = style_text_for_cfg_lang
getLanguage = get_language
isEnglish = is_english
styleTextZhForPolish = style_text_zh_for_polish
styleTextEnForImage = style_text_en_for_image
getRolePolishPrompt = get_role_polish_prompt
getRoleGenerateImagePrompt = get_role_generate_image_prompt
getScenePolishPrompt = get_scene_polish_prompt
getScenePolishPromptSingle = get_scene_polish_prompt_single
getSceneGenerateImagePrompt = get_scene_generate_image_prompt
getSceneGenerateSingleImagePrompt = get_scene_generate_single_image_prompt
getPropExtractionPrompt = get_prop_extraction_prompt
getPropPolishPrompt = get_prop_polish_prompt
formatUserPrompt = format_user_prompt
getStoryboardSystemPrompt = get_storyboard_system_prompt
getStoryboardUserPromptSuffix = get_storyboard_user_prompt_suffix
getUniversalOmniMultiBeatFormatSpec = get_universal_omni_multi_beat_format_spec
getStoryboardUniversalOmniModeSuffix = get_storyboard_universal_omni_mode_suffix
getStoryboardNarrationExtraInstructions = get_storyboard_narration_extra_instructions
getRealisticPhysicalScaleContract = get_realistic_physical_scale_contract
getFirstFramePrompt = get_first_frame_prompt
getKeyFramePrompt = get_key_frame_prompt
getLastFramePrompt = get_last_frame_prompt
getIdentityAnchorsPrompt = get_identity_anchors_prompt
getCharacterExtractionPrompt = get_character_extraction_prompt
getSceneExtractionPrompt = get_scene_extraction_prompt
